...
def _find_legacy_sent_alert(conn, discovery):
    return conn.execute(
        """
        SELECT *
        FROM sent_alerts
    ...
    try:
        ...
        for discovery in discoveries:
            if discovery["id"] in existing:
                continue
            legacy = _find_legacy_sent_alert(conn, discovery) if _table_exists(conn, "sent_alerts") else None
            actions.append({
...
def apply_migration(db_path, backup=True):
    if backup:
        backup_path = f"{db_path}.{MIGRATION_NAME}.{datetime.now().strftime('%Y%m%d%H%M%S')}.bak"
    ...
    try:
        with conn:
            ...
            for discovery in discoveries:
                ...
                legacy = _find_legacy_sent_alert(conn, discovery)
                source_type = "legacy_migrated" if legacy else "reconstructed"
...
def main():
    ...
    for action in plan["actions"]:
        ...
        print(f"- discovery_id={action['discovery_id']} ticker={action['ticker']} source={action['source_type']} legacy_sent_alert_id={legacy}")
